...
import { GeminiAuthenticationProvider } from "./services/auth/providers/GeminiAuthenticationProvider"
...
export async function activate(context: vscode.ExtensionContext) {
	...
	// Initialize Gemini Authentication Provider
	const geminiStatusBarItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 100)
	geminiStatusBarItem.command = 'cline.authenticateGemini'
	const geminiAuthProvider = new GeminiAuthenticationProvider(context, context.secrets, geminiStatusBarItem)
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('cline.authenticateGemini', async () => {
			try {
				await vscode.authentication.getSession('google-gemini', ['https://www.googleapis.com/auth/generative-language'], { createIfNone: true })
				vscode.window.showInformationMessage('Successfully authenticated with Google for Gemini CLI access!')
			} catch (error) {
